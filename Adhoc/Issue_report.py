...
C_RANGE = '#dc2626'   # 이슈 구간 (빨강)
C_LOTS  = '#f59e0b'   # 그중 지정한 랏 (노랑)
C_REST  = '#94a3b8'   # 나머지 (회색)


def _pt_color(p):
    """
    점 하나의 색.
      구간 ∩ 지정 랏 → 노랑   (가장 좁혀 지목한 대상)
      구간만          → 빨강
      랏만(구간 밖)   → 노랑 옅게
      나머지          → 회색
    """
    r, l = p.get('in_range'), p.get('in_lots')
    if l and r:
        return C_LOTS, 3.0, 0.95
    if r:
        return C_RANGE, 2.6, 0.85
    if l:
        return C_LOTS, 2.6, 0.6
    if p.get('in_sel'):        # 구간·랏 정보가 없을 때(옛 응답) 폴백
        return C_RANGE, 2.6, 0.85
    return C_REST, 1.9, 0.55
...
def _chart(it):
    ...
    # 이슈 구간 배경 띠 — 어디를 지목했는지 한눈에
    #   기간을 지정했으면 그 기간, 아니면 선택 집합 기준
    has_range = any(p.get('in_range') for p in ser)
    key = 'in_range' if has_range else 'in_sel'
    spans, run = [], None
    for i, p in enumerate(ser):
        if p.get(key) and run is None:
            run = i
        elif not p.get(key) and run is not None:
            spans.append((run, i - 1)); run = None
    if run is not None:
        spans.append((run, n - 1))
    for a, z in spans:
        x1, x2 = px(a), px(z)
        parts.append(f'<rect x="{x1:.1f}" y="{PAD_T}" '
                     f'width="{max(x2 - x1, 2):.1f}" height="{ih}" '
                     f'fill="{C_RANGE}" opacity="0.06"/>')
    ...
    # 색 범례 — 지정한 랏이 있을 때만 셋으로 나뉜다
    has_lots = any(p.get('in_lots') for p in ser)
    leg = [f'<span class="lg"><i style="background:{C_REST}"></i>나머지</span>']
    if any(p.get('in_range') or p.get('in_sel') for p in ser):
        leg.append(f'<span class="lg"><i style="background:{C_RANGE}"></i>'
                   f'이슈 구간</span>')
    if has_lots:
        leg.append(f'<span class="lg"><i style="background:{C_LOTS}"></i>'
                   f'지정 랏</span>')
    return svg + '<div class="legend">' + ''.join(leg) + '</div>'
...
.cap { font-size:10px; color:var(--mut); font-weight:700;
  font-family:'SF Mono','Consolas',monospace; margin:0 0 4px; }
.legend { display:flex; gap:14px; flex-wrap:wrap; font-size:11px;
  font-weight:700; color:var(--tx2); margin:2px 0 6px; }
...
def build_report(oper_id, lot_cd, sel, unit=None, params=None,
    ...
    ★ unit 을 받지 못하면 'lot' 으로 떨어지는데, 그러면 화면에서는
      웨이퍼 단위로 분석해 놓고 리포트만 랏 평균으로 나온다.
      조용히 넘어가면 원인을 찾기 어려우므로 그 사실을 리포트에 표기한다.

      only_issue=True  이상·주의·구간내 변곡 항목만 (기본)
                       전체를 넣으면 파라미터 수만큼 차트가 생겨 파일이 커진다
      params           특정 파라미터만 지정할 수도 있다
    """
    unit_given = bool(str(unit or '').strip())
    unit = (unit or 'lot').strip()
    if not unit_given:
    ...
    unit_txt = '랏 평균' if unit == 'lot' else '웨이퍼 단위'
    scope_txt = '(이상·주의·변곡만)' if only_issue else '(전체)'
    # 차트에 실제로 몇 점이 찍혔는지 — 단위가 제대로 반영됐는지 확인용
    npts = max((len(i.get('series') or []) for i in items), default=0)
    pt_txt = f'{npts}점' + (' (랏 평균)' if unit == 'lot' else ' (웨이퍼)')
    if not unit_given:
        unit_txt += ' ※ 요청에 단위가 없어 기본값 사용'
...
<div class="meta">
  ...
  <span>데이터 단위 <b>{unit_txt}</b></span>
  <span>차트 점 <b>{pt_txt}</b></span>
