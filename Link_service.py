...
# ★ alias_lot_id 를 함께 가져온다.
#   다른 테이블(SRC·REP·DEF)은 alias_lot_id 로 조인 키를 만드는데
#   여기만 lot_id 를 쓰면, 층 사이에서 랏이 바뀐 웨이퍼는 절대 안 붙는다.
#   이 컬럼이 없는 fab 테이블도 있을 수 있어 실패하면 lot_id 로 되돌린다.
SQL_CHM_UNIT = """        select alias_lot_id, lot_id, wf_id, eqp_id, module_id,
        ...
        group by alias_lot_id, lot_id, wf_id, eqp_id, module_id"""

# alias_lot_id 가 없는 경우의 대체 쿼리
SQL_CHM_UNIT_FALLBACK = """        select lot_id, wf_id, eqp_id, module_id,
               MAX(last_update_dtts) as last_update_dtts
        from lake_catalog.apc.apc_sk_wafer_hst_r2r_all_{unit}
        where dt between '{dt_s}' and '{dt_e}'
          and operation_id like '{oper_pfx}%'
...
def build_chm_sql(link_id, dt_s, dt_e, use_alias=True):
    """
    장비·챔버 이력 조회 SQL.
    ...
    tpl = SQL_CHM_UNIT if use_alias else SQL_CHM_UNIT_FALLBACK
    body = "\n        union\n".join(
        tpl.format(unit=u, dt_s=dt_s, dt_e=dt_e, oper_pfx=pfx)
        for u in CHM_UNITS)
    ...
    cols = ('h.alias_lot_id, h.lot_id' if use_alias else 'h.lot_id')
    return f"""
select {cols}, h.wf_id, h.eqp_id, h.module_id, h.last_update_dtts
from (
{body}
) h{where}
"""
...
def fetch_chm(lake, link, dt_s, dt_e, run_query, on_progress=None):
    ...
    """
    query = build_chm_sql(link['link_id'], dt_s, dt_e, use_alias=True)
    if VERBOSE:
    ...
    try:
        ...
        # alias_lot_id 컬럼이 없는 테이블이면 lot_id 로 다시 시도한다
        print(f"[CHM:{link['alias']}] alias_lot_id 조회 실패 — "
              f"lot_id 로 재시도합니다: {e.__class__.__name__}")
        query = build_chm_sql(link['link_id'], dt_s, dt_e, use_alias=False)
        try:
            d = run_query(lake, query)
        except Exception:
    ...
    if 'last_update_dtts' in d.columns:
        _key = ['alias_lot_id' if 'alias_lot_id' in d.columns else 'lot_id',
                'wf_id']
        d = (d.sort_values('last_update_dtts')
               .drop_duplicates(subset=_key, keep='last'))

    # ★ alias_lot_id 가 있으면 그걸 쓴다 — 다른 테이블과 같은 기준이라야
    #   층 사이에서 랏이 바뀐 웨이퍼도 붙는다.
    lot_col = 'alias_lot_id' if 'alias_lot_id' in d.columns else 'lot_id'
    if lot_col == 'lot_id' and VERBOSE:
        print(f"  [CHM:{link['alias']}] alias_lot_id 가 없어 lot_id 로 "
              f"조인합니다 — 랏이 바뀐 웨이퍼는 안 붙을 수 있습니다")
    d['wkey'] = [cfg2.wafer_key(a, w)
                 for a, w in zip(d[lot_col], d['wf_id'])]
    if on_progress:
        ...
        for tag in jobs:
            ...
            if VERBOSE and added:
                ...
                print(f"  [{tag}:{lk['alias']}] 컬럼 {len(added)}개 추가 · "
                      f"값 있는 웨이퍼 {hit:,}장")
                if hit == 0:
                    # ★ 왜 안 붙었는지 실제 키를 보여 준다.
                    #   '확인하세요' 만으로는 무엇을 확인할지 알 수 없다.
                    _diag_join(out['__wkey'], wide['__wkey'],
                               f"{tag}:{lk['alias']}")

    return out.drop(columns=['__wkey'])


def _diag_join(base_keys, link_keys, label):
    """
    조인이 0건일 때 양쪽 키를 비교해 원인을 알려 준다.

    ★ 흔한 원인 셋
        ① 랏 자릿수가 다르다 (본공정 9자리 vs 연계 7자리)
        ② wf_id 형태가 다르다 (01 vs 1 vs 1.0)
        ③ 애초에 겹치는 랏이 없다 (조회 기간·LOT_CD 가 어긋남)
    """
    b = pd.Series(list(base_keys)).astype(str)
    l = pd.Series(list(link_keys)).astype(str)

    print(f'\n    ── [{label}] 조인 0건 — 키 비교 ' + '-' * 26)
    print(f'    본공정 {len(b):,}개 · 연계 {len(l):,}개')
    print(f'    본공정 예: {list(b.head(4))}')
    print(f'    연계   예: {list(l.head(4))}')

    b_lot = set(b.str.rsplit('.', n=1).str[0])
    l_lot = set(l.str.rsplit('.', n=1).str[0])
    common = b_lot & l_lot

    if not common:
        print(f'    -> 겹치는 랏이 없습니다 (본공정 {len(b_lot)}종 / '
              f'연계 {len(l_lot)}종)')
        bl = sorted({len(x) for x in list(b_lot)[:200]})
        ll = sorted({len(x) for x in list(l_lot)[:200]})
        if bl != ll:
            print(f'    -> 랏 길이가 다릅니다: 본공정 {bl} vs 연계 {ll} '
                  f'— alias_lot_id 자릿수를 확인하세요')
        else:
            print('    -> 조회 기간이나 LOT_CD 가 어긋났을 수 있습니다')
    else:
        print(f'    -> 랏은 {len(common)}종이 겹칩니다 — wf_id 형태 문제입니다')
        one = sorted(common)[0]
        bw = sorted(set(b[b.str.startswith(one + '.')]))[:8]
        lw = sorted(set(l[l.str.startswith(one + '.')]))[:8]
        print(f'    랏 {one} 의 키:')
        print(f'      본공정: {bw}')
        print(f'      연계  : {lw}')
    print('    ' + '-' * 52)
