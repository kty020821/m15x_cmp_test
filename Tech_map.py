...
# ══════════════════════════════════════════════════════════
# 공정 코드 → 공정명
#   구닥스 OPER_DESC 를 웹 프로세스에서 조회할 수 없어 여기서 관리한다.
#   여기에 없는 공정은 드롭박스에 코드만 표시되므로, 필요할 때 한 줄씩 넣으면 된다.
#
#   ★ 아래 목록은 셸에서 한 번에 뽑을 수 있다:
#       import equipment.analysis_service as svc
#       df = svc.get_config()
#       for _, r in df.drop_duplicates(subset=['OPER_ID']).iterrows():
#           print(f"    '{r['OPER_ID']}': '{r['OPER_DESC']}',")
# ══════════════════════════════════════════════════════════
OPER_NAME_MAP = {
    # 'V50700E': 'BLC BUF NIT TOUCH CMP',
}


def oper_name(oper_id):
    """공정명 (없으면 빈 문자열)"""
    return OPER_NAME_MAP.get(str(oper_id).upper(), '')
